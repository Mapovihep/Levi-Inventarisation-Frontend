...
export const sagaReducer = (state = initialState, action) =>{
       switch (action.type) {
        ...
        case SIGN_UP:
           ...
           case LOAD_POSTS:
               let postsFromGetting = action.payload;
               for (let post of postsFromGetting) {
                   if (post.comments === null) {
                       post.comments = [];
                   }
               }
            return {...state, 
                posts: sorting(postsFromGetting),
                loaded: true
            }
        ...
        case ADD_COMMENT: 
            let [currentPostAddCom] = state.posts.filter(post=> post.id===action.payload.modelPostId);
