...
export const Item: React.FC<ItemListProps> = ({itemParams, shortItem: short, addedToUserFlag}) => {
    ...
    const addItemToUser = (E:React.MouseEvent) => {
        ...
        !state.addedToUser ? dispatch(addUserAC("ADD_ITEM", itemParams)) : dispatch(addUserAC("DELETE_ITEM", itemParams))
    }
    ...
    return (<React.Fragment>
        <ul className="itemInfo__ItemList">
            ...
            <li className={`itemParam__ItemList ${short ? "dateBig" : "date"}`}>  {itemParams.createdAt.substring(0,10)}   </li>
            <li className={`itemParam__ItemList ${short ? "priceBig" : "price"}`}>  {itemParams.price}   </li>
            {!short&&<li className="itemParam__ItemList room">  {itemParams.room}   </li>}
